...
parser.add_argument('--grade_up_to', type=int, default=0, help='Will grade stages up to this one (included)')
parser.add_argument('--grade_from', type=int, default=0, help='Will grade stages from this one')
parser.add_argument('--output', type=str, default='', help='Will write the score to this file')
...
class MemMapInstance:
    ...
    def AreProcessAddressesOverlapping(self):
        ...
        return any(v > 64 for _, v in occupancy.items())
...
class MemMapTimeline:
	...
	self.panic = False;
        ...
        for line in lines:
            if line.startswith(PM_PREFIX):
                line_split = filter(bool, line.split(' '))
    ...
    def PerformChecks(self, grade_from, up_to):
        all_functions = [('IsKernelIsolated', 'Kernel isolation', 0),
        ...
        for function_name, message, start_at_tick in to_perform:
            ...
            if res:
                ...
                print '{}{} failed at tick {}{}'.format(BColors.FAIL, message, tick, BColors.ENDC)
            total += res
        if args.output != '':
    ...
    def CheckInvariants(self, expected_max_tick):
        if self.mem_data:
            ...
            if not self.panic and expected_max_tick != max_tick:
                print 'Kernel prematurely exited, expected to last until tick', expected_max_tick, 'but last recorded tick is', max_tick
                exit(1)
        
        for tick, instance in sorted(self.mem_data.items()):
            if not instance.CheckOwnershipInvariant():
                print 'At least one non-kernel page owned by a process is not accessible by it at tick ', tick
                exit(1)

            if not instance.CheckCGAAccessInvariant():
                print 'At least one process does not have access to the CGA console at tick', tick
                exit(1)
